...
module.exports = {
  ...
  get_form_selects: function ( $ ) {
    let fieldsets = $( `form fieldset` );
    let all_groups = [];
    for ( let set of fieldsets ) {
      all_groups.push($( set ).find( `input, .btn` ));
    }
    return all_groups;
  },  // Ends get_form_select

  get_fieldsets: function ( $ ) {
    let form_selects = $( `form select` );
    return form_selects;
  },  // Ends get_fieldset

  get_da_groups: function ( $ ) {
    let form_groups = $( `form .da-form-group` );
    return form_groups;
  },  // Ends get_da_group
  ...
  getAllFields:  async function ( scope, { html }) {
    ...
    *  TODO: Pass in `$` instead? */
    ...
    // TODO: keep multiselect obj in mind
    // Are we not afraid of repeats?
    let all_nodes = $( `#dasigpage,
                        .daquestionactionbutton,
                        form select,
    ...
    // form: select
    // .da-form-group div:not(fieldset)?: select, textarea, input
    // fieldset: input, button, .daquestionactionbutton or button (or .btn)
    // #dasigpage

    // when choosing action, examine the tag/type of the first element and then act accordingly
    
    // // Is there a situation where `select` doesn't have a `.da-form-group` parent? That is implied. Is that the case elsewhere?
    // // What about `field:` instead of `fields:`?
    // // I guess repeats won't harm us, though they are less efficient
    // let all_nodes = $( `#dasigpage, .daquestionactionbutton, form select, .da-form-group, fieldset` );

    let fieldsets = get_fieldsets( $ );

    let form_selects = [];  // get_form_selects( $ );
    let form_groups = [];  // get_da_groups( $ );
    let custom_groups = [];
    let sig = [$( `#dasigpage` )];

    // al_three_parts_date
    // Avoid getting hidden fields
    // Can action buttons be grouped? Otherwise we'll always select the same one

    // Q: Make this a [list] or [list | obj]? A: A [list]
    let node_groups = [ ...fieldsets, ...form_selects, ...form_groups, ...custom_groups, sig ]
    let field_groups = [];

    for ( let group of node_groups ) {
      let field_group = [];
  ...
  uploadFiles: async function ( scope, { handle, answer }) {
    ...
    */
    ...
    let uploads_names = answer.split(`,`);
    if ( uploads_names.length === 0 ) {
      await scope.addToReport( scope, {
        type: `error`,
        value: `Your list of files to upload is empty.`
      });
      return false;  // The row wasn't used
    }

    let dirs = [];
    if ( session_vars.get_origin() === `playground` ) {
      let files_playground = fg.sync(
      ...
      dirs = files_playground.concat( files_s3 );
    } else {
      dirs = fg.sync(
        [ `**/docassemble/**/data/sources` ],
        { onlyFiles: false, suppressErrors: true }
      );
    }

    if ( session_vars.get_debug() ) {
      console.log( `All discovered dirs to upload from:`, dirs );
    }

    // Make sure sources directory exists and has files in it
    if ( dirs.length === 0 ) {
      await scope.addToReport( scope, {
        type: `warning`,
        value: `The test files need to be in the "sources" directory, `
          + `but ALKiln couldn't find the directory or it was empty.`
      });
      return false;  // The row wasn't used
    }
    ...
    for ( let name of uploads_names ) {
     
      let trimmed_name = name.trim();
      let matching_paths = [];

      for ( let one_dir of dirs ) {
        // accumulate all matching paths
        let new_paths = fg.sync([ `${ one_dir }/${ trimmed_name }` ]);
    ...
    if ( missing_files.length > 0 ) {
      await scope.addToReport( scope, {
        type: `warning`,
        value: `ALKiln could not find "${ missing_files.join('", "') }" in any of these folders: "${ dirs.join('", "') }".`
      });
    }

    // If all files missing, return that the row wasn't used
    if ( missing_files.length === uploads_names.length ) {
      return false;  // The row wasn't used
    }
  ...
  draw_signature: async function ( scope, name ) {
    ...
    * takes a while to load. */
    ...
    if ( !canvas ) { await scope.addToReport(scope, { type: `error`, value: msg }); }
  ...
  steps: {
    ...
    set_random_page_vars: async ( scope ) => {
      ...
      // TODO: Authors to use a specific tag to avoid re-trying random tests?
  ...
  set_random_input_for: {
    ...
    input: async function (scope, { field }) {
      ...
      */
      ...
      // TODO: Handle custom datatypes for other types of fields?
  ...
  // # Debugging random inputs
  // Groups of fields (radio, custom dates, checkboxes)
  // Replication (look at guesses and include encoded names to output non-human table)
  // No retries (cucumber tags?)
  // Change order of when screenshot is taken, and css for button we're about to press
