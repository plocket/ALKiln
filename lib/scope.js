...
module.exports = {
  ...
  which_datatype: async function( scope, { handle }) {
  ...
  funnel_the_answer: async function( scope, { handle, field, answer }) {
    ...
    */
    ...
    if ( handle !== null ) {
      ...
      custom_datatype = await scope.which_datatype( scope, { handle });  
    }
  ...
  set_random_input_for: {
    ...
    input: async function (scope, { field }) {
      ...
      */
      ...
      let datatype = await scope.which_datatype( scope, { handle });

      let is_custom_datatype = scope.setCustomDatatype[ datatype ] !== undefined;
      if ( is_custom_datatype ) {
        ...
        if ( hidden_field !== null ) {
          // only set the value for the original field
          if ( type === `hidden` ) {

            // set the value
            let answer = await scope.get_random_input_for[ datatype ]( scope );
